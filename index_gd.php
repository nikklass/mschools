...
if (isset($_REQUEST['tag']) && $_REQUEST['tag'] != ''){
	...
	if ($tag == "login") {
		...
		$student_password = trim($_REQUEST['student_password']); 
		$school_id = trim($_REQUEST['school_id']); 
		
		$user = $db->getUserByUserNamePasswordSchool($student_reg_no, $student_password, $school_id);
	...
	} else if ($tag == "get_points_user_data"){
		
		$id = $_REQUEST['id'];
		
		$user = $db->getPointsUserById($id);
		
		if ($user != false){
			$response["error"]	 = false;
			$response["user"]["full_names"] = $user["fullnames"];
			$response["user"]["cover_name"] = $user["covername"];
			$response["user"]["points_balance"] = $db->format_num($user["pointsbalance"]);	
			echo json_encode($response);
		} else {
			$response["error"]	= true;
			$response["error_msg"] = "Incorrect ID";
			echo json_encode($response);
		}
			...
			//main query
			$queryadd = "SELECT * FROM sch_results WHERE reg_no=$reg_no AND term=$term AND year=$year";
			...
			//main query
			$queryadd = "SELECT * FROM sch_fees WHERE reg_no=$reg_no AND term=$term AND year=$year";
	...
	} else if ($tag == "get_points_usage"){
		$id = $_REQUEST['id'];
		$year = $_REQUEST['year'];
		if (!$year){ $year=date("Y"); }
	
		//$points_data = $db->getCenterVisits($user_id, $year);
		...
		if ($year && $id)
		{
			...
			$sql .= " LEFT JOIN aar_centers ac ON ac.id=cu.center_id ";
			$sql .= " LEFT JOIN aar_user_covers uc ON uc.id=cu.user_cover_id ";
			$sql .= " LEFT JOIN aar_users au ON au.id=uc.user_id ";
			$sql .= " WHERE SUBSTR(cu.visit_at,1,4) = '$year' AND au.id=$id";
			//$sql .= " WHERE au.id=$user_id";
			$mresult = mysql_query($sql);
			//$points_data['sql'] = $sql; 
			...
			while ($row = mysql_fetch_assoc($mresult))
			{
				...
				$pointsUsed = $db->format_num($row['pointsUsed']);
				$visitDate = $db->php_date($row['visitDate']);
				$visitDate = $db->friendly_date($visitDate);
				$points_data[] = array
				(
					'centerName' => $centerName,
					'pointsUsed' => $db->format_num($pointsUsed),
					'visitDate' => $visitDate
				);
			}
