...
<?php
	class DBFunctions {
		...
		public function getUserByUserNamePasswordSchool($student_reg_no, $student_password, $school_id){
			$user_query = "SELECT * FROM sch_students WHERE reg_no='$student_reg_no' AND school_id=$school_id";
			$result = mysql_query($user_query);
			$no_of_rows = mysql_num_rows($result);
			if ($no_of_rows > 0){
				$result = mysql_fetch_array($result);
