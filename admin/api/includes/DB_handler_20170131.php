...
class DbHandler {
	...
	*/
	...
	public function getBulkSmsBalance($sch_id, $user_id, $admin) {
		...
		if ($super_admin || ($admin && $company_ids)) {
			...
			//check bulk sms balance
			$url = "http://41.215.126.10:5333/sms/fxd1.php?usr=" . $usr . "&pass=" . $pass . "&src=" . $src . "&action=check_bal";
			...
			if  ($result->src_ip) {
				...
				$response["src_ip"] = $result->src_ip;
								
			} else {
				
				$sch_name = $this->getSchoolName($sch_id);
				//show error msg
				$response["message"] = "No Bulk SMS Account exists for \"<b>$sch_name</b>\".<br><br> Please contact PendoSchools admin to create one.";
				$response["error_type"] = AN_ERROR_OCCURED_ERROR;
				$response["error"] = true;
	...
	// fetch inbox
    public function fetchSMSInbox($sch_id, $user_id=NULL, $admin=NULL, $start_date=NULL, $end_date=NULL, $user_phone_number=NULL, $page=NULL, $lperpage=NULL, $sort=NULL, $searchPhrase=NULL) {
		...
		if ($super_admin || ($admin && $company_ids)) {
			...
			$url = "http://41.215.126.10:5333/sms/fxd1.php?action=get_school_sms_inbox&usr=" . $usr . "&pass=" . $pass . "&src=" . $src . "&sch_first_name=" . $sch_first_name . "&sort=" . $sort . "&start_date=" . $start_date . "&end_date=" . $end_date . "&source=" . $user_phone_number . "&page=" . $page . "&lperpage=" . $lperpage . "&stxt=" . $searchPhrase;
			...
			if  ($resp->rows) {
				...
				//show success msg
				$response['error'] = false;
				$response['query'] = $resp->query;
				$response['rows'] = $resp->rows;
				$response['total'] = $resp->total;
				$response['rowCount'] = $resp->rowCount;
				$response['current'] = $resp->current;
								
			} else {
				
				$response['rows'] = "";
				$response['url'] = $url;
				$response['sch_first_name'] = $sch_first_name;
				$response['total'] = 0;
				$response['rowCount'] = 0;
				$response['current'] = 0;
    ...
    public function getSchoolData($phone_number, $school_id) {
		...
		//get school data
		$query = "SELECT sch_id, sch_name, sch_first_name, address, province, category, extra, sch_profile, events_calender, phone1, phone2, motto, sms_welcome1, sms_welcome2, sch_paybill_no FROM sch_ussd ";
		$query .= " WHERE sch_id = ? ";
		$stmt = $this->conn->prepare($query);
    ...
    public function getStudentGridListing($sch_id=NULL, $page=NULL, $search_text=NULL, $lperpage=NULL, $sort=NULL, $id=NULL, $admin=NULL, $user_id=NULL, $no_pagination=FALSE, $student_ids=NULL, $current_class=NULL, $stream=NULL) { 
		...
		if ($super_admin || ($admin && $company_ids) || !$admin) {
			...
			if ($student_ids) { $queryMain .= " AND id IN ($student_ids) "; }
			if ($sch_id) { $queryMain .= " AND sch_id=$sch_id "; }
			if ($current_class) { $queryMain .= " AND current_class=$current_class "; }
			if ($stream) { $queryMain .= " AND stream='$stream' "; }
    ...
    public function editSchool($id, $name, $sch_first_name, $sch_level, $sch_category, $province, $sch_county, $status, $motto, $phone1, $phone2, $sms_welcome1, $sms_welcome2, $address, $sch_profile, $sch_paybill_no, $user_id) { 
		...
		if (!$id || !$name){
		...
		} else if ($sch_first_name && $this->schFirstNameExists($sch_first_name, $id)){
			...
			//update results
			$queryMain = "UPDATE sch_ussd SET sch_name = ?, sch_first_name = ?, sch_level = ?, sch_category = ?, sch_province = ?, sch_county = ?, status = ?, motto = ?, sch_profile = ?";
			$queryMain .= ", sch_paybill_no = ?, phone1 = ?, phone2 = ?, sms_welcome1 = ?, sms_welcome2 = ?, address = ?, updated_at = ?, updated_by = ? WHERE sch_id = ? ";
			$stmtMain = $this->conn->prepare($queryMain);
			$stmtMain->bind_param("ssiiiiisssssssssii", $name, $sch_first_name, $sch_level, $sch_category, $province, $sch_county, $status, $motto, $sch_profile, $sch_paybill_no, $phone1, $phone2, $sms_welcome1, $sms_welcome2, $address, $created_at, $user_id, $id);
			$result = $stmtMain->execute();
			if ($result) {	
	...
	*/
	//get user company ids
	function getUserCompanyIds($user_id, $perms, $top_est_id="") {
