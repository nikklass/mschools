...
class DbHandler {
    ...
    public function loginUser($phone_number, $password) {
		...
		//echo "phone_number, password - $phone_number, $password"; exit;
    ...
    public function createUser($phone_number, $password, $email=NULL, $gender=NULL, $full_names=NULL) 
	{
    ...
    public function sendBulkSMS($usr, $pass, $src, $phone_number, $message, $sms_type_id = 1) {
		...
		///////////////////////////////////////////////// enable later ////////////////////////////////////////////////////////////////////////
		//http://41.215.126.10:5333/sms/fxd1.php?usr=13055&pass=GOMBATO&src=707200&dest=0720743211&msg=Another Bulk Test Magicals- Gombato"
		...
		$curl->options['CURLOPT_COOKIEJAR'] = "cookie.txt";
		$curl->options['CURLOPT_COOKIEFILE'] = "cookie.txt";
		$curl->options['CURLOPT_FOLLOWLOCATION'] = 1;
		$curl->options['CURLOPT_REFERER'] = SITEPATH;
		$curl->options['CURLOPT_HTTPHEADER'] = 'User-Agent: Mozilla/5.0 (Windows; U; Windows NT 5.1; en-US; rv:1.8.1.15) Gecko/20080623 Firefox/2.0.0.15';

		$curl->options['CURLOPT_PORT'] = 8089;
	
		$result = $curl->get('http://41.215.126.10:5333/sms/fxd1.php', array(
			'usr' => $usr,
			'pass' => $pass,
			'src' => $src,
			'dest' => $phone_number,
		...
		/*if ($result->error) {
			echo 'Error: ' . $result->errorCode . ': ' . $curl->errorMessage . "\n";
		} else {
			echo 'Data server received via POST:' . "\n";
			var_dump($curl->response->form);
			$result = $curl->response->form;
		}*/
		
		//echo "result - $result == ";
		
		/*echo "error 1 - " . $result->error . " == ";
		echo "error 2 - " . $result['error'] . " == ";
	...
	*/
    ...
    public function createStudentFee($amount, $payment_mode, $paid_by, $paid_at, $student_id, $year) {
		...
		if (!$amount || !$student_id || !$year || !$paid_at || !$payment_mode || !$paid_by) {
	...
	//Upload student data from excel file
    public function uploadStudents($sch_id, $students, $user_id=NULL) {
	...
	//create a new SCHOOL
    public function createSchool($sch_name, $sch_category, $sch_province, $sch_county, $sch_level, $status, $motto, $phone1, $phone2, $sms_welcome1, $sms_welcome2, $address, $paybill_no) {
		...
		if (!$sch_name || !$sch_category) {
			...
			// insert query
			$query = "INSERT INTO sch_ussd(sch_name, sch_category, sch_level, sch_county, sch_province, sch_paybill_no, status, motto, phone1, phone2, sms_welcome1";
			$query .= ", sms_welcome2, address, created_by, created_at) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
			$stmt = $this->conn->prepare($query);
			$stmt->bind_param("siiiiisssssssis", $sch_name, $sch_category, $sch_level, $sch_county, $sch_province, $paybill_no, $status, $motto, $phone1, $phone2, $sms_welcome1, $sms_welcome2, $address, $created_by, $current_date);
			$result = $stmt->execute();
			$stmt->close();
    ...
    public function getStudentGridListing($sch_id=NULL, $page=NULL, $search_text=NULL, $lperpage=NULL, $sort=NULL, $id=NULL, $admin=NULL) { 
		...
		if (SUPER_ADMIN_USER || $company_ids || !$admin) {	
			...
			$queryMain .= ", guardian_address, guardian_phone FROM sch_students WHERE id!='' ";
			if ($search_text) { $queryMain .= " AND ($full_article_search_text) "; }
	...
	// fetch students listing for grid
    public function getStudentResultsGridListing($sch_id, $reg_no=NULL, $year=NULL, $term=NULL, $page=NULL, $search_text=NULL, $lperpage=NULL, $sort=NULL, $student_id=NULL, $user_id=NULL, $single_student_result=NULL) { 
		...
		if ($success) {
			...
			$queryMain .= " WHERE sr.id!='' AND sr.year = $year AND sr.term = $term ";
			if ($student_id) { $queryMain .= " AND sr.student_id = $student_id "; }
			if ($reg_no && $sch_id) { $queryMain .= " AND (sr.reg_no = '$reg_no' AND sr.sch_id = $sch_id ) "; }
			if ($search_text) { $queryMain .= " AND ($full_article_search_text) "; }
	...
	// edit single school
    public function editSchool($id, $name, $sch_level, $sch_category, $province, $sch_county, $status, $motto, $phone1, $phone2, $sms_welcome1, $sms_welcome2, $address, $sch_profile, $sch_paybill_no, $user_id) { 
		...
		if (!$id || !$name){
			...
			//update results
			$queryMain = "UPDATE sch_ussd SET sch_name = ?, sch_level = ?, sch_category = ?, sch_province = ?, sch_county = ?, status = ?, motto = ?, sch_profile = ?";
			$queryMain .= ", sch_paybill_no = ?, phone1 = ?, phone2 = ?, sms_welcome1 = ?, sms_welcome2 = ?, address = ?, updated_at = ?, updated_by = ? WHERE sch_id = ? ";
			$stmtMain = $this->conn->prepare($queryMain);
			$stmtMain->bind_param("siiiiisssssssssii", $name, $sch_level, $sch_category, $province, $sch_county, $status, $motto, $sch_profile, $sch_paybill_no, $phone1, $phone2, $sms_welcome1, $sms_welcome2, $address, $created_at, $user_id, $id);
			$result = $stmtMain->execute();
			if ($result) {	
	...
	public function saveStudentFeeHistory($id, $created_by, $created_at) {
		...
		if (!$id) {
			...
			//print_r($fee_payment_data); exit;
			
			//$id = $fee_payment_data["id"];
			...
			// insert query				
			$query = "INSERT INTO sch_fees_payments_history(fees_id, fees_payments_id, amount, payment_mode, paid_by, paid_at, created_by, created_at) VALUES(?, ?, ?, ?, ?, ?, ?, ?)";
			$stmt = $this->conn->prepare($query);
			//echo "$query - $fees_id, $id, $amount, $payment_mode, $paid_by, $paid_at, $created_by, $created_at"; exit;
			
			if ($stmt = $this->conn->prepare($query)){
				
				$stmt->bind_param("iidsssis", $fees_id, $id, $amount, $payment_mode, $paid_by, $paid_at, $created_by, $created_at);
				$result = $stmt->execute();
				$stmt->close();
	...
	public function saveSchoolDataHistory($id, $created_by, $created_at) {
		...
		if (!$id) {
			...
			// insert query	
			$query = "INSERT INTO sch_ussd_history(sch_ussd_id, sch_name, sch_level, sch_category, sch_province, sch_county, status, motto";
			$query .= ", sch_profile, sch_paybill_no, phone1, phone2, sms_welcome1, sms_welcome2, address, created_at, created_by";
			$query .= ") VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"; 
			//echo "$query";
			
			if ($stmt = $this->conn->prepare($query)){
				$stmt->bind_param("isiiiiisssssssssi", $id, $sch_name, $sch_level, $sch_category, $province, $sch_county, $status, $motto, $sch_profile, $sch_paybill_no, $phone1, $phone2, $sms_welcome1, $sms_welcome2, $address, $created_at, $created_by);
				$result = $stmt->execute();
				$stmt->close();
    ...
    public function editSingleResult($result_item_id, $score, $sch_id) { 
		...
		if (!$this->isDataNumeric($score)){
			...
			if ($result) {	
				...
				//save archive data
				$this->saveResultItemArchiveData($result_item_id, $updated_at, $updated_by);
	...
	public function getStudentFeePaymentsHistory($page=NULL, $user_id=NULL, $lperpage=NULL, $admin=NULL, $id=NULL) {
		...
		if ($success) {
			...
			if (SUPER_ADMIN_USER || $company_ids || !$admin) {*/
				...
				//main query
				$query = "SELECT sfp.id, sfp.fees_id, sfp.amount, pm.name, sf.student_id, sf.year, ss.full_names, sfp.created_at, sfp.paid_by, sfp.paid_at, c.full_names";
				...
				$query .= " WHERE sfp.fees_payments_id = $id ";
				...
				if ($stmt = $this->conn->prepare($query)){
					...
					$stmt->execute();
					$stmt->store_result();
					$stmt->bind_result($payment_id, $payment_fees_id, $amount, $payment_mode, $student_id, $year, $student_name, $created_at, $paid_by, $paid_at, $created_by);
					/* fetch values */
					while ( $stmt->fetch() ) {
						...
						$tmp["payment_amount_fmt"] = $this->format_num($amount);
						$tmp["payment_amount_fmt2"] = "Kshs. " . $this->format_num($amount);
						...
						$tmp["payment_created_at_fmt"] = $this->adjustDate("d/m/Y", $this->php_date($created_at), NULL);
	...
	public function getStudentFeePayments($page=NULL, $student_id=NULL, $phone_number=NULL, $year=NULL, $user_id=NULL, $sch_id=NULL, $reg_no=NULL, $lperpage=NULL, $admin=NULL, $id=NULL) {
		...
		if ($success) {
			...
			if (SUPER_ADMIN_USER || $company_ids || !$admin) {*/
				...
				//main query
				$query = "SELECT sfp.id, sfp.fees_id, sfp.amount, pm.name, pm.code, sf.student_id, sf.year, ss.full_names, sfp.created_at, sfp.paid_by, sfp.paid_at, c.full_names";
				...
				if ($sch_id && !$reg_no) { $query .= " AND sf.sch_id = $sch_id "; }
				//if ($sch_id && ($sch_id!=1) && (!($reg_no || $student_id))) { $query .= " AND sf.sch_id IN ($sch_id) "; }
				...
				if ($stmt = $this->conn->prepare($query)){
					...
					$stmt->execute();
					$stmt->store_result();
					$stmt->bind_result($payment_id, $payment_fees_id, $amount, $payment_mode, $payment_mode_code, $student_id, $year, $student_name, $created_at, $paid_by, $paid_at, $created_by);
					/* fetch values */
					while ( $stmt->fetch() ) {
    ...
    function getSchoolGridListing($page=NULL, $user_id=NULL, $search_text=NULL, $lperpage=NULL, $sort=NULL, $id=NULL, $admin=NULL,  $company_ids=NULL, $show_all=NULL) { 
		...
		if (SUPER_ADMIN_USER || $company_ids || !$admin) {
			...
			//Get the records
			$queryMain = "SELECT s.sch_id, s.sch_name, address, sch_province, p.name, sch_category, sch_level, sl.name, extra, sch_profile";
			...
			$stmt->bind_result($sch_id, $sch_name, $address, $province, $province_name, $category_id, $level, $level_name, $extra, $sch_profile, $phone1, $phone2, $motto, $sms_welcome1, $sms_welcome2, $sch_paybill_no, $status, $status_name, $county_id, $county_name, $category_name);
			/* fetch values */
			while ( $stmt->fetch() ) {
				$tmp = array();
				$tmp["sch_id"] = $sch_id;
    ...
    public function getStudentFees($sch_id, $student_id, $year, $reg_no, $show_student_fees) {
		...
		if ($success) {
			...
			$query .= " ON sf.student_id = st.id WHERE sf.student_id = $student_id "; 
			if ($sch_id && $reg_no) { $query .= " AND sf.sch_id = $sch_id AND sf.reg_no = '$reg_no' "; }
			...
			if ($stmt = $this->conn->prepare($query)) {
				...
				if ($result) {	
					...
					$tmp['total_fees'] = "Kshs. " . $this->format_num($total_fees, 0);
					$tmp['fees_bal'] = "Kshs. " . $this->format_num($fees_bal, 0);
					$tmp['fees_paid'] = "Kshs. " . $this->format_num($fees_paid, 0);
	...
	public function subscribeUser($phone_number, $school_id, $reg_no) {
		...
		//echo "sch_name - $sch_name - $school_id - $province - $phone_number, $reg_no"; exit;
	...
	private function isUserActivated($input) {
		$email = $input;
		$phone_number = $this->formatPhoneNumber($input);
		$query = "SELECT id FROM clients WHERE (email = ? OR phone_number = ?) AND status = " . ACTIVE_STATUS;	
		$stmt = $this->conn->prepare($query);
        $stmt->bind_param("ss", $email, $phone_number);
        $stmt->execute();
        $stmt->store_result();
	...
	function saveResultItemArchiveData($result_item_id, $updated_at, $updated_by)
	{
